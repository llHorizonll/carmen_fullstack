...
export function ApInvoiceViewPage() {
    ...
    return (
      <div className="space-y-6">
        <div className="flex items-center gap-4">
          ...
          <h1 className="text-2xl font-bold">{t("ap.invoices.view.notFound")}</h1>
        </div>
        <Card>
          <CardContent className="pt-6">
            <p className="text-muted-foreground">{t("ap.invoices.view.notFoundDesc")}</p>
    ...
    <div className="space-y-6">
          ...
          <div>
            <div className="flex items-center gap-3">
              ...
              <Badge variant={getStatusVariant(invoice.status)}>
                {apInvoiceStatusLabels[invoice.status]}
              </Badge>
            </div>
          ...
          {isDraft && (
            <>
              <Button
                variant="outline"
                size="sm"
                onClick={() => navigate(`/ap/invoices/${id}/edit`)}
              >
      ...
      <div className="grid gap-6 lg:grid-cols-3">
        ...
        <Card className="lg:col-span-2">
          <CardHeader>
            <CardTitle>{t("ap.invoices.view.invoiceDetails")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              <div>
                <Label className="text-muted-foreground">{t("ap.invoices.view.vendorInvoice")}</Label>
                <p className="font-mono">{invoice.vendorInvoiceNumber || "-"}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">{t("ap.invoices.view.invoiceDate")}</Label>
                <p>{format(new Date(invoice.invoiceDate), "dd MMM yyyy")}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">{t("ap.invoices.view.dueDate")}</Label>
                <p className={new Date(invoice.dueDate) < new Date() && invoice.balanceAmount > 0 ? "text-destructive font-medium" : ""}>
                  {format(new Date(invoice.dueDate), "dd MMM yyyy")}
                </p>
              </div>
              <div>
                <Label className="text-muted-foreground">{t("ap.invoices.view.currency")}</Label>
                <p>{invoice.currencyCode}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">{t("ap.invoices.view.exchangeRate")}</Label>
                <p className="font-mono">{invoice.exchangeRate}</p>
              </div>
              <div>
                <Label className="text-muted-foreground">{t("ap.invoices.view.fiscalPeriod")}</Label>
                <p>{invoice.fiscalPeriodName || "-"}</p>
              </div>
              {invoice.reference && (
                <div>
                  <Label className="text-muted-foreground">{t("ap.invoices.view.reference")}</Label>
                ...
                <div className="space-y-2">
                  {invoice.approvedAt && (
                    <div className="flex items-center gap-2 text-sm">
                      ...
                      <span>{t("ap.invoices.view.approvedBy")}: {invoice.approvedBy}</span>
                  ...
                  {invoice.rejectionReason && (
                    <div className="rounded-md bg-destructive/10 p-3">
                      <p className="text-sm font-medium text-destructive">{t("ap.invoices.view.rejectionReason")}:</p>
                      <p className="text-sm">{invoice.rejectionReason}</p>
                    </div>
                  )}
                  {invoice.voidReason && (
                    <div className="rounded-md bg-destructive/10 p-3">
                      <p className="text-sm font-medium text-destructive">{t("ap.invoices.view.voidReason")}:</p>
            ...
            {invoice.journalVoucherId && (
              <>
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span className="text-sm text-muted-foreground">{t("ap.invoices.view.journalVoucher")}:</span>
                  <Button
                    variant="link"
                    className="h-auto p-0"
                    onClick={() => navigate(`/gl/journal-vouchers/${invoice.journalVoucherId}`)}
                  >
                    {t("ap.invoices.view.viewJournalVoucher")}
                  </Button>
                </div>
              </>
            )}
        ...
        <Card>
          <CardHeader>
            <CardTitle>{t("ap.invoices.view.amounts")}</CardTitle>
          </CardHeader>
          <CardContent>
            <div className="space-y-3">
              <div className="flex justify-between">
                <span className="text-muted-foreground">{t("ap.invoices.view.subTotal")}</span>
                <span className="font-mono">{formatCurrency(invoice.subTotal, invoice.currencyCode)}</span>
              </div>
              {invoice.discountAmount > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">{t("ap.invoices.view.discount")}</span>
              ...
              {invoice.tax1Amount > 0 && (
                <div className="flex justify-between">
                  <span className="text-muted-foreground">
                    {invoice.tax1ProfileName || t("ap.invoices.view.vat")} ({invoice.tax1Rate}%)
                  </span>
      ...
      <Card>
        <CardHeader>
          <CardTitle>{t("ap.invoices.view.lineItems")} ({invoice.lines.length})</CardTitle>
        </CardHeader>
        <CardContent>
            ...
            <Table>
              <TableHeader>
                <TableRow>
                  ...
                  <TableHead>{t("ap.invoices.view.account")}</TableHead>
                  <TableHead>{t("ap.invoices.view.department")}</TableHead>
                  <TableHead>{t("common.description")}</TableHead>
                  <TableHead className="text-right">{t("ap.invoices.view.quantity")}</TableHead>
                  <TableHead className="text-right">{t("ap.invoices.view.unitPrice")}</TableHead>
                  <TableHead className="text-right">{t("ap.invoices.view.amount")}</TableHead>
                  {invoice.lines.some(l => l.tax1Amount > 0) && (
                    <TableHead className="text-right">{t("ap.invoices.view.tax")}</TableHead>
                  )}
                </TableRow>
              </TableHeader>
              <TableBody>
      ...
      <Dialog open={rejectDialogOpen} onOpenChange={setRejectDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("ap.invoices.reject.description", { invoiceNumber: invoice.invoiceNumber })}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
      ...
      <Dialog open={voidDialogOpen} onOpenChange={setVoidDialogOpen}>
        <DialogContent>
          <DialogHeader>
            ...
            <DialogDescription>
              {t("ap.invoices.void.description", { invoiceNumber: invoice.invoiceNumber })}
            </DialogDescription>
          </DialogHeader>
          <div className="py-4">
