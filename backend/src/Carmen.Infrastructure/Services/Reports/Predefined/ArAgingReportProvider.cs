...
public class ArAgingReportProvider : IPredefinedReportProvider
{
    private readonly CarmenDbContext _context;

    public ArAgingReportProvider(CarmenDbContext context)
    {
        _context = context;
    }

    public PredefinedReportType ReportType => PredefinedReportType.ArAging;
    ...
    public async Task<ReportDataSet> GenerateAsync(Dictionary<string, string> parameters)
    {
        ...
        var invoices = await _context.ArInvoices
            ...
            .Where(i => (i.Status == ArInvoiceStatus.Approved || i.Status == ArInvoiceStatus.PartiallyPaid)
                        && i.BalanceAmount > 0)
            .OrderBy(i => i.Customer.CustomerCode)
        ...
        return new ReportDataSet(
            ...
            Columns:
            [
                new("CustomerCode", "Customer Code", ColumnType.Text, 80, AggregateFunction.None),
                new("CustomerName", "Customer Name", ColumnType.Text, 150, AggregateFunction.None),
            ...
            Groups:
            [
                new("CustomerCode", "Customer", true),
            ],
